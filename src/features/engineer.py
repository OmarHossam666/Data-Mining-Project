...
# __file__ resolves to src/features/engineer.py, so parent.parent gets us to 'src/'
...
def extract_and_join_data():
    ...
    # SQL Query joining Fact with all Dimensions (checkpoint_week now from dim_semester only)
    query = """
    SELECT 
        f.fact_id, f.gpa, f.absences, f.risk_score, f.risk_label,
        sem.checkpoint_week,
    ...
    # Convert checkpoint_week from String to numeric for filtering
    df['checkpoint_week'] = pd.to_numeric(df['checkpoint_week'], errors='coerce')
...
def generate_correlation_heatmap(df):
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "feature_correlation_heatmap.png")
    plt.close()
    logger.info("Saved feature_correlation_heatmap.png to results/")
    return df_corr
...
def process_checkpoint(df, checkpoint_week, name):
    ...
    # Drop any rows that don't have a target label
    ...
    # Fill any remaining NaNs in features with 0 (assignment, not inplace)
    X = X.fillna(0)
    ...
    # Save the scaler alongside model artifacts
    joblib.dump(scaler, config.MODELS_DIR / f"scaler_{name}.joblib")
